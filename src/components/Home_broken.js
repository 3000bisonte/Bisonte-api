...
const Home = () => {
  ...
  useEffect(() => {
    console.log('🏠 Home: Estado sesión:', { loading, session: !!session, isClient });
  ...
  const handleLogout = async () => {
    ...
    signOut(); // Usar signOut del hook useMobileSession
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#e3dfde] via-[#f8fafc] to-[#41e0b3]/10 flex flex-col items-center w-full relative overflow-hidden">
      ...
      {/* Header con contraste y sombra (centrado y estable) */}
      <header
        className="sticky top-0 z-30 bg-[#18191A] border-b-4 border-[#41e0b3] shadow-xl animate-fade-in-down w-full"
      ...
      {/* Bienvenida elegante debajo del header */}
      ...
      {/* Espacio para header */}
  {/* Eliminado spacer fijo; el flujo ahora se ajusta al header */}
